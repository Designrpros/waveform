...
const Container = styled.div`
  max-width: 800px; margin: 4rem auto; padding: 2rem; color: ${({ theme }: { theme: DefaultTheme }) => theme.text};
`;
...
const PublicBadge = styled.span`
  ...
  background-color: ${({ theme }: { theme: DefaultTheme }) => theme.buttonHoverBg};
  border: 1px solid ${({ theme }: { theme: DefaultTheme }) => theme.borderColor};
  color: ${({ theme }: { theme: DefaultTheme }) => theme.subtleText};
`;
const PlaylistDescription = styled.p`
  font-size: 1.125rem; color: ${({ theme }: { theme: DefaultTheme }) => theme.subtleText};
...
const Message = styled.p`
  text-align: center; color: ${({ theme }: { theme: DefaultTheme }) => theme.subtleText};
`;
const BackButton = styled(Link)`
  display: inline-flex; align-items: center; gap: 0.5rem; color: ${({ theme }: { theme: DefaultTheme }) => theme.accentColor}; 
  text-decoration: none; font-weight: 500; margin-bottom: 2rem;
  &:hover { color: ${({ theme }: { theme: DefaultTheme }) => theme.text}; }
`;
